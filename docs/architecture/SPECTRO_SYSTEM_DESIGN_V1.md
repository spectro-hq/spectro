...
9. Product API and console data surfaces. The bounded, authorized ClickHouse event-list query and Console event explorer are implemented under ADR-041. Error issue aggregation and occurrence drill-down follow under ADR-042. PostgreSQL-backed issue lifecycle state follows under ADR-043, with lifecycle history, status filtering, and ordered migrations under ADR-044.
10. Session investigation reuses the bounded event query's exact session filter. Performance overview aggregation follows under ADR-045 and network overview aggregation under ADR-046; both drill back into the same event and session evidence. Release health then aggregates existing contextual signals under ADR-047 without inventing a composite score and drills into the same evidence surfaces.
